    ...
    <div className="h-screen bg-white font-outfit border border-gray-200 overflow-hidden flex flex-col shadow-none rounded-3xl text-gray-900">
      ...
      >
        ...
        <div className="relative">
          <button 
            ...
            className="p-2 bg-brand-500 text-white rounded-lg shadow-theme-sm hover:bg-brand-600 transition-all focus:outline-none"
        ...
        <div className="flex items-center gap-1.5 ml-auto" style={{ WebkitAppRegion: 'no-drag' }}>
          <button onClick={handleAddNote} className="p-1.5 text-gray-900 hover:bg-gray-100 rounded-lg transition-colors focus:outline-none">
            <svg className="size-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
            </svg>
          </button>
          <button 
            className="px-2 py-0.5 text-gray-900 font-bold text-3xl leading-none hover:bg-gray-100 rounded-lg transition-colors focus:outline-none" 
            onClick={() => window.minimize()}
          >
            -
          </button>
          <button className="p-1.5 text-gray-900 hover:bg-red-50 hover:text-red-600 rounded-lg transition-all focus:outline-none" onClick={() => window.close()}>
            <svg className="size-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {(searchQuery || statusFilter !== 'All') && (
        <div className="px-6 pb-4 flex items-center gap-3 bg-white border-b border-gray-100" style={{ WebkitAppRegion: 'no-drag' }}>
            {/* Clear All Button LAST */}
          <button 
            onClick={handleClearFilters}
            className="flex items-center gap-2 text-rose-500 hover:text-rose-600 transition-colors text-theme-sm font-bold group"
          >
          ...
          {/* Status Pill FIRST */}
          {statusFilter !== 'All' && (
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-brand-500 text-white rounded-lg text-[11px] font-bold shadow-sm">
          ...
          {/* Search Pill */}
          {searchQuery && (
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 text-gray-500 rounded-lg text-[11px] font-bold shadow-sm">
