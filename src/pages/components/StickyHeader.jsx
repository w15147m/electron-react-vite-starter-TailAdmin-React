...
const StickyHeader = ({ 
  ...
  return (
    <div 
      className="px-6 py-3 flex items-center gap-3 bg-white z-10"
      style={{ WebkitAppRegion: 'drag' }}
    >
      {/* Search Bar */}
      <div className="flex-1 max-w-[280px] flex items-center bg-white border border-gray-200 rounded-lg overflow-hidden" style={{ WebkitAppRegion: 'no-drag' }}>
        <input 
          type="text" 
          placeholder="Search title . . ." 
          className="flex-1 px-4 py-1.5 border-none focus:ring-0 outline-none text-[13px] placeholder:text-gray-300"
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
        />
        <button className="px-2.5 py-1 border-l border-gray-100 bg-white hover:bg-gray-50 transition-colors">
